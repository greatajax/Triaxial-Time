...
\section{The Structure of the Temporal Volume}
\label{sec:threeaxes}
...
Each axis governs a distinct functional role in the phase field's behaviour. Together they define the kinematic and dynamical substrate from which all observed phenomena emerge.
...
\begin{enumerate}
    ...
    The only fundamental field is a scalar phase field $\phi(T_1,T_2,T_3)$ defined everywhere on the temporal volume—here termed the \emph{score}. All structure arises from variation in this field.
    ...
    \item \textbf{Planck Tone.}  
    Phase gradients are bounded by the universal Planck tone $\kappa$: $|\nabla\phi| \le \kappa$—the irreducible tension that stabilises integer windings and enforces emergent discretization.
    
    \item \textbf{No External Lawgiver.}  
    There are no imposed equations of motion acting on the system from outside. All lawful behaviour emerges from internal constraints, symmetries, and relaxation dynamics of the phase field.
\end{enumerate}
...
The baseline state of the score is uniform phase:
\[
\nabla \phi = 0.
\]
This state corresponds to complete silence: no structure, no persistence, no distinction. Importantly, this state is not “nothing.” The score remains fully present, fully extended, and fully capable of supporting structure. Uniformity is absence of differentiation, not absence of being.

Structure emerges only where phase varies. All phenomena correspond to notes (transient variations), tones (persistent circulations), or more complex resonant configurations built from them. There is no additional substance beneath the phase field. Phase is not carried by something else; it is the thing.

\subsection{Symmetry and Invariance}

The score admits the following fundamental symmetries:
...
\begin{itemize}
    ...
    \item \textbf{Gauge freedom} in absolute phase: only phase differences are physically meaningful, just as only relative pitch matters in a silent room.
\end{itemize}

These symmetries replace the role traditionally played by spacetime covariance. Conservation laws arise not from imposed symmetries of space, but from invariances of the score itself.
...
Closed paths, resonant loops, and braided patterns are possible only in a space of sufficient dimensionality. Variation in $\phi$ along compact directions gives rise to persistent circulatory patterns—structures that are inherently periodic and resonant rather than geometric or material. The three-dimensional score is the minimal structure capable of supporting stable identities, memory, coexistence of possibilities, and irreversible advance.

\subsection{Emergence Without Projection}

Nothing in the score resembles space, distance, or geometry as ordinarily conceived. There are no points, no metrics, and no neighborhoods in the spatial sense. Nevertheless, relational structure exists: proximity, orientation, and direction are defined internally through phase relationships and coherence constraints.

What is later perceived as space arises only when a moving two-dimensional section through the score is mistaken for an arena of existence rather than recognised as an interface. This misidentification is systematic, lawful, and unavoidable for embedded observers, which explains both the success and the limits of spatial description.

The remainder of this work develops the internal structure of the score, defines its axes explicitly, and demonstrates how matter, fields, gravitation, quantum phenomena, and mind arise as necessary consequences—not additions—to this primitive.

\section{Primacy of Harmonic Structure}

The primitive elements introduced above—a continuous three-dimensional score with compact directions, a single scalar phase field, and a universal bound $\kappa$ on gradient tension—admit one natural class of stable configurations: persistent circulations of phase along the compact axes.

Once such circulation exists, harmonic structure arises immediately and inescapably. Integer windings enforce discrete frequencies; bounded gradients stabilise resonant alignments; superpositions of commensurate circulations produce richer, phase-locked patterns. These are not secondary features—they are the direct mathematical consequences of the primitives.

Conventional descriptions already make extensive use of this periodic, phase-based mathematics (Fourier decomposition, normal modes, dispersion relations, complex amplitudes), yet almost always embed it within a prior spatial or material framework. The present work takes the opposite stance: the circulatory and resonant behaviour is primary; spatial and material concepts arise only later, upon projection onto moving perceptual slices.

To describe this primary level with maximum precision and minimal extraneous assumption, the following structural terms are adopted:

\begin{itemize}
    \item \textbf{Note} — a local, transient phase variation ($\nabla\phi \ne 0$, typically open propagation).
    \item \textbf{Tone} — a persistent, topologically protected circulation of phase (integer winding along a compact temporal direction); a \emph{pure tone} emphasises a single-frequency circulation without overtones.
    \item \textbf{Phasor} — the formal representation of uniform circulation (e.g., $\phi = \omega T_k + \theta_0$), underlying pure tones and serving as the rotating complex vector whose projections yield observed trigonometric behaviour.
    \item \textbf{Chord} — a coherent resonant superposition of tones whose circulation spectra are phase-locked and occupy overlapping regions in the $T_2$--$T_3$ plane.
    \item \textbf{Motif} — a recognisable, topologically linked or braided pattern formed by multiple tones or chords (natural locus of fermionic statistics and composite structure).
    \item \textbf{Overtone} — a higher-frequency nested circulation within a tone, chord, or motif.
    \item \textbf{Resonance} — stable phase alignment between separate tones or motifs.
    \item \textbf{Dissonance} — phase misalignment that increases gradient tension and drives irreversible relaxation along $T_1$.
    \item \textbf{Staff} — a single $T_1$=constant perceptual slice on which concurrent phase relations are registered.
    \item \textbf{Score} — the complete three-dimensional temporal volume containing every simultaneity and development.
\end{itemize}
...
\hline
Structure & Definition & Emergent Role & Stability Mechanism \\
\hline
Note & Local phase variation ($\nabla\phi \ne 0$) & Transient fields, open propagations & None (relaxes along $T_1$) \\
Tone & Closed circulation ($\oint d\phi = 2\pi n$) & Persistent entities, particles & Compact topology + $\kappa$ \\
Chord & Resonant superposition of tones & Composites, interactions & Spectral overlap \\
Motif & Linked/braided tones or chords & Fermions, exotics & Linking invariants \\
Overtone & Nested higher circulation & Excitations, spectra & Harmonic matching \\
\hline
\end{tabular}
\caption{Taxonomy of Primary Phase Structures}
\label{tab:taxonomy}
\end{table}

No aesthetic claim is made. No appeal to subjective auditory experience is intended. The language is chosen solely for its precision and economy.

Older spatial terms—“particle”, “field”, “world-line”, “curved manifold”, “wave packet”—are retained only where they serve as effective descriptions of projected behaviour on the moving staff. They are not wrong within their domain, but they are ontologically secondary.

The score is the complete temporal volume; dissonance dissipates monotonically as its pages turn. What we experience as the physical world is the ordered reading of successive staves by finite, embedded performers. The harmonic terminology is not metaphorical. It is exact.
...
A persistent resonant structure—here termed a \emph{tone}—corresponds to uniform phase circulation along a compact temporal axis. The simplest such structure is described by
\[
\phi(T_k) = \omega T_k + \theta_0 \pmod{2\pi},
\]
where $T_k$ is a compact axis (typically $T_3$) and $\omega$ is integer-valued to ensure single-valuedness after one full cycle.

This configuration is precisely a \emph{phasor}: a rotating complex vector of constant magnitude in the complex plane. The phasor representation makes explicit that the circulation is literal and internal to the score; no spatial circle or geometric rotation is presupposed.

When an advancing staff intersects a circulating phasor, orthogonal components of the circulation are registered on the staff as
\[
\cos(\omega T_k + \theta_0), \quad \sin(\omega T_k + \theta_0).
\]
These projections are the origin of all trigonometric structure in observed physics. The identity $\cos^2 + \sin^2 = 1$ expresses conservation of resonant intensity under projection.

Composite tones and chords arise from superpositions of multiple phasors with commensurate frequencies and stable relative phases. Their vector sum forms a more complex but still rigid rotating figure, preserving topological protection while enriching the projected behaviour.\subsection*{Projection and Trigonometric Structure}
